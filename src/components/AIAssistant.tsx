...
import { getCurriculumAdvice } from '@/src/services/geminiService';
...
export default function AIAssistant() {
  ...
  const handleGetAdvice = async () => {
    ...
    const result = await getCurriculumAdvice(userProgress, IE_SUBJECTS);
    setAdvice(result);
    setLoading(false);
  };
              ...
              {loading ? (
                ...
                <div className="prose prose-sm max-w-none text-foreground/80 font-medium leading-relaxed">
                  {advice ? (
                    ...
                    <p className="text-foreground/40 italic">Click the button to get personalized academic advice.</p>
                  )}
                </div>
              )}
            </div>
